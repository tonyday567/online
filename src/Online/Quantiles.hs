...
data Quantiles f a = (Ord a, Abelian a, Semiring a) => Quantiles
  { qSize    :: Int
...
-- online quantiles
quantiles :: (Semiring a, Ord a, Group a, Field a) => Int -> (a -> a) -> a -> L.Fold a [a]
...
digitize :: (BoundedField a) => Int -> (a -> a) -> a -> L.Fold a Int
digitize n f decay = L.Fold step begin extract
  where
    begin = (Quantiles n empty empty, negInfinity)
...
stepP2 :: (Ring a, Field a) => (Quantiles Vector a) -> a -> (Quantiles Vector a)
stepP2 q@(Quantiles b ms cs) a
  | length ms < b =
        Quantiles b
        (fromList (sort $ toList $ cons a ms))
        (generate (fromIntegral $ length ms + one) (\x -> fromIntegral x + one))
  ...
  where
    ...
    csIdeal = generate b (\i -> one + ((last counts') - one) * fromIntegral i/(fromIntegral b - one))
    d = zipWith (-) csIdeal counts'
    dsign = map (\x -> if x>zero then one else zero) d
    c' = zipWith (-) (tail counts') (init counts')
    isOut = zipWith3 (\d' nd1 nd0 -> d' >= one && nd1 > one  || d' <= (negate one) && nd0 > one) (init $ tail d) (tail c') c'
...
addOne :: (Quantiles Vector a) -> a -> (Quantiles Vector a)
addOne (Quantiles b ms qs) a
    | a < ms!0 =
      Quantiles b (cons a (tail ms))
      (cons one (map (+one) $ tail qs))
    | a > last ms =
      Quantiles b
      (snoc (init ms) a)
      (qs // [(b - 1, one + qs!(b - 1))])
    | otherwise =
      Quantiles b
      ms
      (imap (\i q -> if ms!i > a then q+one else q) qs)
...
p2 :: (Rg a, Field a) => Vector a -> Vector a -> Vector a -> Vector a
p2 q n d = res
  where
    d'  = init $ tail d
...
l2 :: (Rg a, Field a, Ord a) => Vector a -> Vector a -> Vector a -> Vector a
l2 q n d =
  zipWith6
  (\q'' d'' qd' nd' qd'' nd'' ->
    ((+) q''
     (if d''>zero then d'' * qd' / nd' else d'' * qd'' / nd'')))
